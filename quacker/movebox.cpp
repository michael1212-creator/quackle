...
MoveBox::MoveBox(QWidget *parent) : View(parent) {
  ...
  QStringList headers;
  headers << tr("Move") << tr("Score") << tr("Leave") << tr("Win %")
          << tr("Valuation");
...
void MoveBox::moveActivated(QTreeWidgetItem *item) {
  ...
  for (QMap<Quackle::Move, QTreeWidgetItem *>::iterator it = m_moveMap.begin();
       it != m_moveMap.end(); ++it) {
    if (it.value() == item) {
...
void MoveBox::removeMove() {
  ...
  for (const auto &it : selectedItems) {
    for (QMap<Quackle::Move, QTreeWidgetItem *>::iterator mapIt =
             m_moveMap.begin();
         mapIt != m_moveMap.end(); ++mapIt) {
      if (mapIt.value() == it) {
...
void MoveBox::setMoves(const Quackle::MoveList &moves,
  ...
  if (m_previousMoves.size() == moves.size()) {
    ...
    for (Quackle::MoveList::const_iterator it = moves.begin(); it != end;
         ++it, ++prevIt) {
      if (!(*prevIt == *it)) {
        resorted = true;
        break;
...
#pragma clang diagnostic pop
  ...
  for (Quackle::MoveList::const_iterator it = moves.begin(); it != end; ++it) {
    QMap<Quackle::Move, QTreeWidgetItem *>::const_iterator mapEnd(
        m_moveMap.end());
    for (QMap<Quackle::Move, QTreeWidgetItem *>::const_iterator mapIt =
             m_moveMap.begin();
         mapIt != mapEnd; ++mapIt) {
      if (mapIt.key() == *it) {
        mapIt.value()->setText(WinPercentageColumn,
                               formatWinPercentage((*it).win));
        mapIt.value()->setText(EquityColumn, formatValuation((*it).equity));

        if (resorted) {
          m_treeWidget->addTopLevelItem(m_treeWidget->takeTopLevelItem(
              m_treeWidget->indexOfTopLevelItem(mapIt.value())));
        }
  ...
  if (resorted) {
    for (QMutableMapIterator<Quackle::Move, QTreeWidgetItem *> mapIt(m_moveMap);
         mapIt.hasNext();) {
      mapIt.next();
...
QTreeWidgetItem *MoveBox::createItem(const Quackle::Move &move) {
  ...
  item->setText(LeaveColumn,
                QuackleIO::Util::letterStringToQString(
                    QuackleIO::Util::arrangeLettersForUser(m_rack - move)));
...
void MoveBox::setSelectionWatchingEnabled(bool enabled) {
  if (enabled) {
    ...
    // QTreeWidgetItem *)), this, SLOT(moveActivated(QTreeWidgetItem *)));
  } else {
    disconnect(m_treeWidget, SIGNAL(itemSelectionChanged()), this,
               SLOT(selectionChanged()));
    disconnect(m_treeWidget, SIGNAL(itemClicked(QTreeWidgetItem *, int)), this,
               SLOT(moveActivated(QTreeWidgetItem *)));
    // disconnect(m_treeWidget, SIGNAL(currentItemChanged(QTreeWidgetItem *,
    // QTreeWidgetItem *)), this, SLOT(moveActivated(QTreeWidgetItem *)));
  }
}
