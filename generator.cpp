...
Generator::Generator(const GamePosition &position) : m_position(position) {}
...
void Generator::filterOutDuplicatePlays() {
  ...
  for (MoveList::iterator it = m_moveList.begin(); it != m_moveList.end();) {
    ...
    if (usedTiles.size() == 1) {
      ...
      for (LetterString::const_iterator letterIt = tiles.begin();
           letterIt != tiles.end(); ++letterIt, ++actualTileIndex)
        if ((*letterIt) != QUACKLE_PLAYED_THRU_MARK)
          break;

      const int row = (*it).startrow + ((*it).horizontal ? 0 : actualTileIndex);
      const int column =
          (*it).startcol + ((*it).horizontal ? actualTileIndex : 0);
      int key = row + QUACKLE_MAXIMUM_BOARD_SIZE * column +
                (QUACKLE_MAXIMUM_BOARD_SIZE * QUACKLE_MAXIMUM_BOARD_SIZE) *
                    tiles[actualTileIndex];
    ...
    if (QUACKLE_ALPHABET_PARAMETERS->isSomeLetter(board().letter(row, col))) {
      ...
      if (row > 0) {
        for (int i = row - 1; i >= 0; i--) {
          if (!QUACKLE_ALPHABET_PARAMETERS->isSomeLetter(
                  board().letter(i, col))) {
            i = -1;
          } else {
            LetterString newpre;
            newpre += QUACKLE_ALPHABET_PARAMETERS->clearBlankness(
                board().letter(i, col));
            newpre += pre;
            pre = newpre;
          }
      ...
      if (row < board().height() - 1) {
        for (int i = row + 1; i < board().height(); i++) {
          if (!QUACKLE_ALPHABET_PARAMETERS->isSomeLetter(
            ...
            suf += QUACKLE_ALPHABET_PARAMETERS->clearBlankness(
                board().letter(i, col));
          }
        }
      }
      ...
      UVcout << QUACKLE_ALPHABET_PARAMETERS->userVisible(pre) << " / "
...
#ifdef DEBUG_GENERATOR
      UVcout << "board().vcross[" << row << "][" << col
             << "] = " << cross2string(board().vcross(row, col)) << endl;
#endif
    }
  }
...
#ifdef DEBUG_GENERATOR
      UVcout << "board().hcross[" << row << "][" << col
             << "] = " << cross2string(board().hcross(row, col)) << endl;
#endif
    }
  }
}
...
void Generator::makeMove(const Move &move, bool regenerateCrosses) {
  ...
  if (move.horizontal) {
    ...
    for (int col = move.startcol; col <= endcol; col++) {
      if (!QUACKLE_ALPHABET_PARAMETERS->isSomeLetter(
              board().letter(row, col))) {
        int upempty = -1;
        for (int hookrow = row - 1; hookrow >= 0; hookrow--) {
          if (!QUACKLE_ALPHABET_PARAMETERS->isSomeLetter(
                  board().letter(hookrow, col))) {
            upempty = hookrow;
            hookrow = -1;
          }
        }
...
void Generator::readFromDawg(int index, unsigned int &p, Letter &letter,
                             bool &t, bool &lastchild, bool &british,
...
bool Generator::checksuffix(int i, const LetterString &suffix) {
  ...
  //#ifdef DEBUG_BOARD
  // UVcout << QUACKLE_ALPHABET_PARAMETERS->userVisible(suffix) << " i: " << i
  // << ", p: " << p << ", c: " << QUACKLE_ALPHABET_PARAMETERS->userVisible(c)
  // << ", sc: " << QUACKLE_ALPHABET_PARAMETERS->userVisible(sc) << endl;
  //#endif
...
LetterBitset Generator::gaddagFitbetween(const LetterString &pre,
                                         const LetterString &suf) {
  // 	UVcout << "fit "
  ...
  for (const GaddagNode *node = sufNode->firstChild(); node;
...
LetterBitset Generator::fitbetween(const LetterString &pre,
                                   const LetterString &suf) {
  ...
  for (Letter c = QUACKLE_FIRST_LETTER;
       c <= QUACKLE_ALPHABET_PARAMETERS->lastLetter(); c++) {
    /*
                    UVcout << "Let's check " <<
                              QUACKLE_ALPHABET_PARAMETERS->userVisible(pre) <<
...
void Generator::gordongoon(int pos, char L, LetterString word,
                           const GaddagNode *node) {
  ...
  if (pos <= 0) {
    ...
    if ((leftcol >= 0) && (leftrow >= 0)) {
      if (!QUACKLE_ALPHABET_PARAMETERS->isSomeLetter(
              board().letter(currow, curcol)) &&
          QUACKLE_ALPHABET_PARAMETERS->isSomeLetter(
              board().letter(leftrow, leftcol))) {
        roomtoleft = false;
      }
    ...
    if (node->isTerminal() && (roomtoleft) && (m_laid > 0)) {
      // UVcout << "found a word or something " << word << " at " << pos <<
      // endl;
      Move move;
      ...
      // UVcout << "found a move: " << move << " score: " << move.score << ",
      // equity: " << move.equity << " outputted by leftmoving loop" << endl;
    }
    ...
    // UVcout << "rightsquare: " << (char)(rightcol + 'A') << rightrow + 1 <<
    // endl;

    if ((rightcol <= board().width() - 1) &&
        (rightrow <= board().height() - 1)) {
      if (QUACKLE_ALPHABET_PARAMETERS->isSomeLetter(
              board().letter(rightrow, rightcol))) {
        roomtoright = false;
        // UVcout << "can't record " << word << " here because of the " <<
        // board().letter(rightrow, rightcol) << endl;
      } else {
        // UVcout << "yay! " << (char)(rightcol + 'A') << rightrow + 1 << " is
        // empty!" << endl; UVcout << board() << endl;
      }
    } else {
...
void Generator::gordongen(int pos, const LetterString &word,
                          const GaddagNode *node) {
  // UVcout << "gordongen(" << pos << ", " << word << ", " << i << ")" << "
  // horiz: " << m_gordonhoriz << endl;
  ...
  if (QUACKLE_ALPHABET_PARAMETERS->isSomeLetter(
          board().letter(currow, curcol))) {
    // UVcout << "gordongen sez a letter (" << board().letter(currow, curcol) <<
    // ") already on this square" << endl;

    Letter boardc = QUACKLE_ALPHABET_PARAMETERS->clearBlankness(
        board().letter(currow, curcol));
    ...
    for (const GaddagNode *child = node->firstChild(); child;
    ...
    if (m_counts[QUACKLE_BLANK_MARK] >= 1) {
      for (const GaddagNode *child = node->firstChild(); child;
        ...
        if (cross.test(childLetter - QUACKLE_FIRST_LETTER)) {
          ...
          // UVcout << "    yeah that'll work" << endl;
          gordongoon(pos,
                     QUACKLE_ALPHABET_PARAMETERS->setBlankness(childLetter),
                     word, child);
          m_counts[QUACKLE_BLANK_MARK]++;
          m_laid--;
        }
...
void Generator::extendright(const LetterString &partial, int i, int row,
                            int col, int edge, int righttiles,
                            bool horizontal) {
...
#ifdef DEBUG_GENERATOR
  UVcout << "extendright(" << QUACKLE_ALPHABET_PARAMETERS->userVisible(partial)
         << ", " << i << ", " << counts2string() << ", " << rowpos << ", "
         << colpos << ", " << horizontal << ")" << endl;
#endif

  if (!QUACKLE_ALPHABET_PARAMETERS->isSomeLetter(
          board().letter(rowpos, colpos))) {
    if (m_counts[c] >= 1) {
      LetterBitset cross;
      if (horizontal) {
      ...
      if (cross.test(c - QUACKLE_FIRST_LETTER)) {
        if (t) {
          ...
          if (couldend) {
            ...
            // i added this because m_laid is wrong and i don't want to break
            // anything by fixing it :) will need to remember to add this bit to
            // the DAGGAD code when we start using it again
...
#ifdef DEBUG_GENERATOR
              UVcout << "found a move: " << move << " laid: " << m_laid
                     << ", score: " << move.score << ", equity: " << move.equity
                     << endl;
#endif
            }
          }
      ...
      if (cross.test(c - QUACKLE_FIRST_LETTER)) {
        if (t) {
          ...
          if (couldend) {
            Move move;
            move.action = Move::Place;
            move.setTiles(partial +
                          QUACKLE_ALPHABET_PARAMETERS->setBlankness(c));
            if (horizontal) {
              move.startrow = row;
              move.startcol = col - (partial.length() - righttiles);
            } else {
        ...
        if (dirpos < edgeDirpos) {
          m_counts[QUACKLE_BLANK_MARK]--;
          m_laid++;
          extendright(partial + QUACKLE_ALPHABET_PARAMETERS->setBlankness(c), p,
    ...
    if (c == boardc) {
#ifdef DEBUG_GENERATOR
      UVcout << "We're playing with "
             << QUACKLE_ALPHABET_PARAMETERS->userVisible(partial)
             << " through a letter (" << boardc << ") or trying to" << endl;
#endif

      bool endofthrough = false;

      if (dirpos < edgeDirpos) {
        extendright(partial + (Letter)QUACKLE_PLAYED_THRU_MARK, p, row, col, 0,
                    righttiles + 1, horizontal);

#ifdef DEBUG_GENERATOR
        UVcout << "  next square is " << board().letter(rownext, colnext)
               << endl;
#endif
...
#ifdef DEBUG_GENERATOR
            UVcout << "found a move: " << move << " which has equity "
                   << move.equity << endl;
#endif
          }
        }
...
void Generator::leftpart(const LetterString &partial, int i, int limit, int row,
                         int col, int edge, bool horizontal) {
#ifdef DEBUG_GENERATOR
  UVcout << "leftpart(" << QUACKLE_ALPHABET_PARAMETERS->userVisible(partial)
         << ", " << i << ", " << counts2string() << ", " << row << ", " << col
         << ", " << edge << ")" << endl;
#endif
  ...
  if (limit > 0) {
    ...
    if (m_counts[QUACKLE_BLANK_MARK] >= 1) {
      m_counts[QUACKLE_BLANK_MARK]--;
      m_laid++;
      leftpart(partial + QUACKLE_ALPHABET_PARAMETERS->setBlankness(c), p,
               limit - 1, row, col, 0, horizontal);
      m_counts[QUACKLE_BLANK_MARK]++;
      m_laid--;
    }
...
Move Generator::generate() {
  ...
  for (int row = 0; row < board().height(); row++) {
    for (int col = 0; col < board().width(); col++) {
      ...
      if (!QUACKLE_ALPHABET_PARAMETERS->isSomeLetter(
              board().letter(row, col)) &&
          !board().vcross(row, col).all()) {
        if (col == 0) {
          anchor = true;
        } else if (!QUACKLE_ALPHABET_PARAMETERS->isSomeLetter(
                       board().letter(row, col - 1))) {
          anchor = true;
        }
      } else if (QUACKLE_ALPHABET_PARAMETERS->isSomeLetter(
                     board().letter(row, col))) {
      ...
      if (anchor) {
        int k = 0;
        for (int i = col - 1; i >= 0; i--) {
          // UVcout << "board().vcross[" << row << "][" << i << "] = " <<
          // board().vcross(row, i) << endl;

          if (!QUACKLE_ALPHABET_PARAMETERS->isSomeLetter(
                  board().letter(row, i)) &&
              board().vcross(row, i).all()) {
            if (i == 0) {
              k++;
            } else if (!QUACKLE_ALPHABET_PARAMETERS->isSomeLetter(
                           board().letter(row, i - 1))) {
              k++;
            }
          } else {
            i = -1;
...
#ifdef DEBUG_GENERATOR
        UVcout << "looking horizontally with the "
               << QUACKLE_ALPHABET_PARAMETERS->userVisible(
                      board().letter(row, col))
               << " at " << row + 1 << (char)(col + 'A') << endl;
        UVcout << "Apparently there are " << k << " empty spots to our left"
               << endl;
#endif
      ...
      if (!QUACKLE_ALPHABET_PARAMETERS->isSomeLetter(
              board().letter(row, col)) &&
          !board().hcross(row, col).all()) {
        if (row == 0) {
          anchor = true;
        } else if (!QUACKLE_ALPHABET_PARAMETERS->isSomeLetter(
                       board().letter(row - 1, col))) {
          anchor = true;
        }
      } else if (QUACKLE_ALPHABET_PARAMETERS->isSomeLetter(
                     board().letter(row, col))) {
      ...
      if (anchor) {
        ...
        for (int i = row - 1; i >= 0; i--) {
          ...
          if (!QUACKLE_ALPHABET_PARAMETERS->isSomeLetter(
                  board().letter(i, col)) &&
              board().hcross(i, col).all()) {
            if (i == 0) {
              k++;
            } else if (!QUACKLE_ALPHABET_PARAMETERS->isSomeLetter(
                           board().letter(i - 1, col))) {
              k++;
            }
          } else {
            i = -1;
  ...
  for (int row = 0; row < board().height(); row++) {
    for (int col = 0; col < board().width(); col++) {
      ...
      if (anchor) {
        ...
        // UVcout << "Apparently there are " << k << " empty spots to our left"
        // << endl; leftpart("", 1, k, row, col, 0, true);

        // UVcout << "looking horizontally with the " << board().letter(row,
        // col) <<
        //         " at " << row + 1 << (char)(col + 'A') << endl;
      ...
      if (anchor) {
        ...
        // UVcout << "Apparently there are " << k << " empty spots to our left"
        // << endl; leftpart("", 1, k, row, col, 0, false);
...
void Generator::spit(int i, const LetterString &prefix, int flags) {
  ...
  if (m_counts[c] >= 1) {
    ...
    if (t) {
      // UVcout << QUACKLE_ALPHABET_PARAMETERS->userVisible(prefix) << c <<
      // endl;

      bool usedAll = true;
      if (!(flags & NoRequireAllLetters)) {
        for (int j = 0;
             j < QUACKLE_FIRST_LETTER + QUACKLE_MAXIMUM_ALPHABET_SIZE; ++j) {
          if (m_counts[j] > 0) {
            usedAll = false;
            break;
          }
  ...
  if (!(flags & ClearBlanknesses && m_counts[c] >= 1)) {
    if (m_counts[QUACKLE_BLANK_MARK] >= 1 || flags & AddAnyLetters) {
      ...
      if (t) {
        // UVcout << prefix << QUACKLE_ALPHABET_PARAMETERS->setBlankness(c) <<
        // endl;
...
void Generator::wordspit(int i, const LetterString &prefix, int flags) {
  ...
  // UVcout << "wordspit(" << i << ", " <<
  // QUACKLE_ALPHABET_PARAMETERS->userVisible(prefix) << ", " << flags << ")" <<
  // endl;
...
void Generator::gaddagAnagram(const GaddagNode *node,
                              const LetterString &prefix, int flags) {
  for (const GaddagNode *child = node->firstChild(); child;
       child = child->nextSibling()) {
    Letter childLetter = child->letter();
...
Move Generator::anagram() {
  ...
  m_spat.clear();
  if (QUACKLE_LEXICON_PARAMETERS->hasGaddag()) {
    gaddagAnagram(QUACKLE_LEXICON_PARAMETERS->gaddagRoot(), LetterString(),
                  NoRequireAllLetters);
  } else {
  ...
  for (WordList::const_iterator it = m_spat.begin(); it != end; ++it) {
    for (unsigned int k = 0; k < (*it).length(); k++) {
      ...
      move.startcol =
          (QUACKLE_BOARD_PARAMETERS->startColumn() - (*it).length() + 1) + k;
...
void Generator::storeWordInfo(WordWithInfo *wordWithInfo) {
  ...
  wordWithInfo->probability =
      Bag::probabilityOfDrawingFromFullBag(wordWithInfo->wordLetterString);

  m_wordspat.clear();
  wordspit(1, LetterString(), 0);

  vector<WordWithInfo>::const_iterator end = m_wordspat.end();
  for (vector<WordWithInfo>::const_iterator it = m_wordspat.begin(); it != end;
       ++it) {
